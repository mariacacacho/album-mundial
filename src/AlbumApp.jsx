...
export default function AlbumApp({ username, initialOwned, onLogout }) {
  ...
  // Debounced sync to server (800ms after last change)
  useEffect(() => {
    clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => {
      saveStickers([...owned]).catch(() => {});
    }, 800);
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.showing}>Mostrando {numbers.length} estampas</div>
        ...
        {numbers.map((n) => {
          ...
          return (
            <button
              key={n}
              onClick={() => toggle(n)}
              style={{ ...styles.stamp, ...(has ? styles.stampOwned : styles.stampMissing) }}
...
const styles = {
  ...
  stampOwned: {
    ...
    color: "#fff", boxShadow: "0 2px 8px rgba(99,102,241,0.5)",
  },
  stampMissing: {
    background: "rgba(255,255,255,0.06)",
